...
class InstantEntry(Entry):
    ...
    def check_entry_criteria_and_update_metadata_and_status(self, kite, tick_last_price, trade, inst_token, timestamp, tick_map):
        ...
        # if metadata['close_count_above_entry_price'] >= 2:
        # if trade.exit_first_target_price < (1.8 * trade.entry_end_price - trade.exit_stop_loss_price) / 0.8:
        #     trade.exit_first_target_price = round((1.8 * trade.entry_end_price - trade.exit_stop_loss_price) / 0.8, 1)
        # super().update_stop_loss_for_normal_entry(trade, metadata, timestamp)
        if metadata['close_count_above_entry_price'] >= 2:
            trade.order_status = 'NOT_PLACED_ENTRY_ALLOWED'
        # if metadata['close_count_below_stop_loss_price'] >= 2:
        #     trade.order_status = 'CANCELLED'
        if 'negative_position' in metadata and metadata['negative_position']:
            trade.order_status = 'CANCELLED'
    ...
    def process_prices_and_quantities(self, trade):
        ...
        if curr_first_target_price < first_target_price_1_to_1_ror:
            curr_first_target_price = first_target_price_1_to_1_ror
        ...
        trade.exit_stop_loss_price = round(trade.entry_end_price * (1 - (real_stop_loss_percent / 100)), 1)
